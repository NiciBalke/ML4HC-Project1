...
class ContrastivePretrainer(BaseSSLEncoder, TokenAugmentationMixin, RepresentationMonitoringMixin, InfoNCELossMixin):
    ...
    def pretrain(self, train_loader: DataLoader, val_loader: DataLoader, device: torch.device, epochs: int, lr: float):
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                emb1 = self.model.encode(v1_ids, v1_mask)
                emb2 = self.model.encode(v2_ids, v2_mask)
                proj1 = self.model.project(emb1)
                proj2 = self.model.project(emb2)

                loss = self._info_nce_loss(proj1, proj2, self.temperature)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                train_loss += float(loss.item())
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    proj1 = self.model.project(self.model.encode(v1_ids, v1_mask))
                    proj2 = self.model.project(self.model.encode(v2_ids, v2_mask))
                    val_loss += float(self._info_nce_loss(proj1, proj2, self.temperature).item())

            val_loss /= max(1, len(val_loader))
            self.val_losses.append(val_loss)

            epoch_msg = f"Epoch {epoch + 1}/{epochs} | InfoNCE Train Loss: {train_loss:.4f} | InfoNCE Val Loss: {val_loss:.4f}"
            if (epoch + 1) % self.monitor_every == 0:
                rep = self._monitor_representation_quality(self.model, val_loader, device)
                rep["epoch"] = epoch + 1
                self.rep_monitor.append(rep)
...
class HybridPretrainer(BaseSSLEncoder, TokenAugmentationMixin, RepresentationMonitoringMixin, InfoNCELossMixin):
    ...
    def pretrain(self, train_loader: DataLoader, val_loader: DataLoader, device: torch.device, epochs: int, lr: float):
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                proj1 = self.model.project(self.model.encode(v1_ids, v1_mask))
                proj2 = self.model.project(self.model.encode(v2_ids, v2_mask))
                contrast_loss = self._info_nce_loss(proj1, proj2, self.temperature)

                loss = self.reconstruction_weight * recon_loss + self.contrastive_weight * contrast_loss
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    total_loss = self.reconstruction_weight * recon_loss + self.contrastive_weight * contrast_loss
...
def q31_pretrain_and_linear_probe(
    ...
    val_embeds, val_labels = get_embeddings(model, val_loader, device)
